...
- On a unified database (`searchMode: "unified"` in the output) use `unified.llmPrompt` as the context and cite with its bracketed labels (`[1]`, `[R1]`, `[P1]`); `unified.chunks[]`, `unified.graph[]` and `unified.forcefulRelations[]` are the structured form.
